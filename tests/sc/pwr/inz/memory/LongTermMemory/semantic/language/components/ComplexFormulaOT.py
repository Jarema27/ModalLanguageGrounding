...
class ComplexFormulaOTTest(unittest.TestCase):
    ...
    def test_get_complementary_formulas(self):
        self.assertEqual(self.cf1.get_complementary_formulas(), [ComplexFormulaOT([self.object_type, self.object_type2],
                                                                                  [self.s1, self.s1], LogicalOperator.
                                                                                  AND)
                         , ComplexFormulaOT([self.object_type, self.object_type2], [self.s1, self.s2], LogicalOperator.
                                            AND)
                         , ComplexFormulaOT([self.object_type, self.object_type2], [self.s2, self.s1], LogicalOperator.
                                            AND)
                         , ComplexFormulaOT([self.object_type, self.object_type2], [self.s2, self.s2], LogicalOperator.
                                            AND)])
